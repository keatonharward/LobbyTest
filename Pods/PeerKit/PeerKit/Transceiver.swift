...
public class Transceiver: SessionDelegate, browserDelegate {
    ...
    func joinLocalSession(withHost peer: MCPeerID, withData data: Data? = nil, mcBrowser: MCNearbyServiceBrowser) {
        mcBrowser.invitePeer(peer, to: session.mcSession, withContext: data, timeout: 30)
    }
    
    public func discovered(discoveredPeer: MCPeerID, withObjects: [String : AnyObject]?) {
        didDiscover(hostWithID: discoveredPeer, withInfo: withObjects as AnyObject)
    }
