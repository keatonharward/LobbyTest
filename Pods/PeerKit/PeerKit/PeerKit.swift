...
public var onDiscovery: ObjectBlock?
...
func didDiscover(hostWithID hostPeerID:MCPeerID, withInfo info:AnyObject?) {
    if let onDiscovery = onDiscovery {
        DispatchQueue.main.async {
            onDiscovery(hostPeerID, info)
        }
    }
}
...
public func joinLocalSession(withHost host:MCPeerID, fromBrowser browser:MCNearbyServiceBrowser, withData data: Data?) {
    transceiver.joinLocalSession(withHost: host, withData: data, mcBrowser: browser)
}
